...
Deno.serve(async (req) => {
  ...
  const { data: tok } = await admin.from('signal_ingest_tokens')
    .select('owner_user_id, profile_id').eq('token', token).maybeSingle();
  if (!tok) return json({ error: 'bad token' }, 401);
  await admin.from('signal_ingest_tokens').update({ last_used_at: new Date().toISOString() }).eq('token', token);
  ...
  try {
    if (type === 'posts' && Array.isArray(data)) {
      ...
      if (rows.length) await admin.from('signal_posts').upsert(rows, { onConflict: 'profile_id,li_post_urn' });
    ...
    if (type === 'post_metrics' && Array.isArray(data)) {
      ...
      for (const m of data as any[]) {
        ...
        await admin.from('signal_posts').upsert({
    ...
    if (type === 'post_demographics' && Array.isArray(data)) {
      ...
      for (const dim of dims) {
        await admin.from('signal_audience_demographics').delete().eq('profile_id', profileId).eq('dimension', dim);
      }
      const rows = clean.map((d) => ({
        profile_id: profileId, dimension: d.dimension, value: String(d.value), count: n(d.count) ?? 0,
      }));
      if (rows.length) await admin.from('signal_audience_demographics').insert(rows);
      return json({ ok: true, written: rows.length });
    }

    if (type === 'profile_snapshot' || type === 'profile_identity') {
      const d = (data ?? {}) as any;
      if (d.headline) await admin.from('signal_profiles').update({ headline: d.headline }).eq('id', profileId);
      const fc = n(d.follower_count);
      if (fc != null) {
        await admin.from('signal_follower_snapshots').upsert({
